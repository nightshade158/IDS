...
def checksum(cksumtype, data, key=None):
    if cksumtype == RSA_MD5:
        ...
        # From RFC 4757 section 4. The RC4-HMAC Kerberos Encryption Types Used by Microsoft Windows
        Ksign = HMAC.new(key, 'signaturekey\0').digest()
        tmp = MD5.new('\x11\x00\x00\x00' + data).digest()
        sys.stderr.write('  Data size : %d\n' % len(tmp))
        sys.stderr.write('  Key: %s\n' % hexlify(Ksign)) 
        checksum_data = HMAC.new(Ksign, tmp)
        sys.stderr.write('  Checksum : %s\n' % checksum_data.hexdigest())
        return checksum_data.digest()
    else:
