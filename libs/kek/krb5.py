...
from libs.pyasn1.codec.der.decoder import decode
from libs.pyasn1.codec.der.encoder import encode
from libs.pyasn1.type.char import GeneralString
from libs.pyasn1.type.namedtype import NamedTypes, NamedType, OptionalNamedType
...
class EncKDCRepPart(Sequence):
    componentType = NamedTypes(
        ...
        OptionalNamedType('encrypted-pa-data', _c(12, SequenceOf(componentType=PAData()))))
...
def build_tgs_req(target_realm, target_service, target_host,
                  ...
                  nonce, current_time, authorization_data=None, pac_request=None, etype=RC4_HMAC):
    ...
    req_body = build_req_body(target_realm, target_service, target_host, nonce, authorization_data=enc_ad, etype=etype)
...
def build_as_req(target_realm, user_name, key, current_time, nonce, pac_request, etype = RC4_HMAC):
    req_body = build_req_body(target_realm, 'krbtgt', target_realm, nonce, user_name, None, etype)
